...
/**
 * Motor de captura genérico sobre MediaPipe Tasks Vision (manos, pose o
 * rostro). Cada módulo aporta un `extraer` que convierte los landmarks de un
 * cuadro en la muestra que le interesa (una distancia, un centroide, un
 * conjunto de puntos…), y un `simular` para el modo sin cámara. El hook se
 * ocupa de la cámara, del modelo, del overlay y de acumular las muestras
 * mientras `isRecording` está activo.
 *
 * Usado por: UPDRS III asistida, expresión facial global y modo programador DBS.
 */

export type ModeloVision = 'MANO' | 'POSE' | 'CARA';
export type Lado = 'DERECHA' | 'IZQUIERDA';

export interface Punto { x: number; y: number }
export interface LandmarkRaw { x: number; y: number; z?: number; visibility?: number }
interface Categoria { categoryName: string; score: number }

interface ResultadoDetector {
  landmarks?: LandmarkRaw[][];
  faceLandmarks?: LandmarkRaw[][];
  handedness?: Categoria[][];
}
interface Detector { detectForVideo: (video: HTMLVideoElement, ts: number) => ResultadoDetector }
...
const MODELOS: Record<ModeloVision, string> = {
  ...
  CARA: 'https://storage.googleapis.com/mediapipe-models/face_landmarker/face_landmarker/float16/1/face_landmarker.task',
};

/**
 * Detectores cacheados a nivel de módulo: en una evaluación se pasa de una
 * tarea a otra muchas veces y recargar el modelo (varios MB + WASM) cada vez
 * haría el examen lento.
 */
...
function obtenerDetector(modelo: ModeloVision): Promise<Detector> {
  if (!cacheDetectores[modelo]) {
    cacheDetectores[modelo] = (async () => {
      ...
      if (modelo === 'CARA') {
        return (await vision.FaceLandmarker.createFromOptions(fs, { baseOptions, runningMode: 'VIDEO', numFaces: 1 })) as unknown as Detector;
      }
 ...
 */
function elegirMano(res: ResultadoDetector, lado: Lado): LandmarkRaw[] | null {
...
export const CONEXIONES_POSE: [number, number][] = [
  [11, 12], [11, 23], [12, 24], [23, 24], [23, 25], [25, 27], [27, 29], [29, 31], [27, 31],
  [24, 26], [26, 28], [28, 30], [30, 32], [28, 32], [11, 13], [13, 15], [12, 14], [14, 16],
  [15, 19], [16, 20],
];

/** true si todos los índices tienen visibilidad suficiente (sólo Pose la informa). */
export const visibles = (raw: LandmarkRaw[], idx: number[], umbral = 0.5) =>
  idx.every(i => raw[i] && (raw[i].visibility ?? 1) > umbral);

export interface OpcionesCaptura<T> {
  modelo: ModeloVision;
  lado: Lado;
  isRecording: boolean;
  isMockMode: boolean;
  /** Landmarks del cuadro en píxeles + crudos (para visibilidad). null = cuadro inválido. */
  extraer: (lm: Punto[], raw: LandmarkRaw[]) => T | null;
  simular: (t: number) => T;
  nodos: number[];
  onDataCollected: (m: (T & { t: number })[]) => void;
}

export function useVisionCapture<T extends object>(op: OpcionesCaptura<T>) {
  const { modelo, lado, isRecording, isMockMode } = op;
  ...
  const muestrasRef = useRef<(T & { t: number })[]>([]);
  const inicioRef = useRef<number | null>(null);
  const detectadoRef = useRef(false);
  // Las funciones del módulo se leen por ref: no reinician el bucle si cambian de identidad.
  ...
  useEffect(() => {
    if (isRecording) {
      ...
      opRef.current.onDataCollected([...muestrasRef.current]);
    }
  }, [isRecording]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const det = await obtenerDetector(modelo);
        if (!activo) return;
    ...
    return () => { activo = false; };
  }, [isMockMode, modelo, iniciarCamara, detenerCamara]);
  ...
  useEffect(() => {
    ...
    let raf = 0;
    let ultimoTs = 0;
    const conexiones = modelo === 'MANO' ? CONEXIONES_MANO : modelo === 'POSE' ? CONEXIONES_POSE : [];

    const registrar = (ahora: number, m: T | null) => {
      if (m && inicioRef.current !== null) muestrasRef.current.push({ ...m, t: (ahora - inicioRef.current) / 1000 });
    };
    ...
    const loop = () => {
      ...
      if (isMockMode) {
        ...
        if (inicioRef.current !== null) registrar(ahora, opRef.current.simular((ahora - inicioRef.current) / 1000));
      } else {
        ...
        if (video && det && video.readyState >= 2) {
          ...
          ultimoTs = ts;
          let raw: LandmarkRaw[] | null = null;
          try {
            const res = det.detectForVideo(video, ts);
            raw = modelo === 'MANO' ? elegirMano(res, lado)
              : modelo === 'CARA' ? (res.faceLandmarks?.[0] ?? null)
              : (res.landmarks?.[0] ?? null);
          } catch { raw = null; }

          if (raw) {
            const lm: Punto[] = raw.map(p => ({ x: p.x * W, y: p.y * H }));
            if (modelo === 'CARA') {
              ctx.fillStyle = 'rgba(255,255,255,0.28)';
              for (const p of lm) { ctx.beginPath(); ctx.arc(p.x, p.y, 1, 0, 2 * Math.PI); ctx.fill(); }
            } else {
            ...
            for (const i of opRef.current.nodos) {
              if (lm[i]) { ctx.beginPath(); ctx.arc(lm[i].x, lm[i].y, modelo === 'CARA' ? 3.5 : 5, 0, 2 * Math.PI); ctx.fill(); }
            }
            const muestra = opRef.current.extraer(lm, raw);
